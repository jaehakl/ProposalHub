...
- **LyricSync Japan 기술 개발 계획**
    

**2. 시스템 아키텍처**

- **구조:** 클라이언트(SPA) - 서버(API) 분리형 아키텍처를 채택한다.
    
- **클라이언트:**
    
    - **형태:** 단일 페이지 애플리케이션 (SPA, Single Page Application)으로 개발하여 부드러운 사용자 경험을 제공한다.
        
    - **역할:** UI 렌더링 및 사용자 상호작용 처리에 집중한다. 모든 데이터 처리 및 비즈니스 로직은 서버 API를 통해 수행한다.
        
- **서버:**
    
    - **형태:** RESTful API 서버로 구축한다.
        
    - **역할:** 형태소 분석, 데이터베이스 조회/저장, 사용자 인증 등 모든 핵심 로직을 담당한다.
...
- **구현 방식:** 사용자 요청 시 실시간으로 생성하지 않고, 사전에 일괄적으로 생성하여 데이터베이스와 스토리지에 저장해두는 방식을 사용한다.
    ...
    - **음성 생성:**
        ...
        - **기술:** VITS, Coqui TTS 등 오픈소스 TTS 모델을 활용한다.
...
- **클라우드 스토리지 (e.g., AWS S3):**
    
    - **역할:** 생성된 음성 파일 및 이미지 파일을 저장한다.
...
- **캐싱:** 반복적인 형태소 분석 요청 및 DB 조회를 줄이기 위해, 서버 단에 캐싱 전략을 도입한다. (e.g., Redis)
