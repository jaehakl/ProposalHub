...
(참고로, 각 문항과 잠재 특성 간 가중치 설계는 상당 부분 인간 전문가의 이론적 추론에 의존하지만, 추후 충분한 응답 데이터가 축적되면 **통계적 기법(예: 요인분석, IRT)** 을 통해 가중치를 보정할 계획도 고려 중이다. 즉, 현 단계에서는 이론 주도적으로 문항을 만들되, 궁극적으로는 데이터에 기반한 **문항 반응 이론**적 분석으로 모델을 고도화할 수 있다.)
